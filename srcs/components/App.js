...
function App() {
  ...
  function handleUpdateUser(user) {
    ...
    myApi.pushNewUserInfo(userName, userAbout).catch((err) => {
      console.log(err);
    }).finally(()=> {
  ...
  function handleCardDelete(card) {
    myApi.removeCard(card._id).catch((err) => {
      console.log(err);
    }).then(() => {
  ...
  function handleCardLike(card, user, callback) {
    ...
    if (isLiked) {
      myApi.deleteLike(card._id, isLiked).catch((err) => {
        console.log(err);
      }).then((newCard) => {
      ...
      myApi.addLike(card._id, !isLiked).catch((err) => {
        console.log(err);
      }).then((newCard) => {
        setCards((state) => state.map((c) => c._id === card._id ? newCard : c))
        callback(newCard.likes.length)
  ...
  function handleUpdateAvatar(link) {
    myApi.pushNewAvatar(link).catch((err)=> {
    console.log(err);
    }).finally(()=> {
  ...
  function handleAddPlaceSubmit(name, link) {
    myApi.pushCard(name, link).catch((err) => {
      console.log(err);
    }).then((res)=> {
      setCards([res, ...cards]);
      closeAllPopup();
  ...
  return (
    ...
    <div className="page">
      <Header/>
      <Main onEditProfile={handleEditProfileClick} onAddPlace={handleAddPlaceClick} onEditAvatar={handleEditAvatarClick} onCardClick={handleCardClick} onCardLike={handleCardLike} onCardRemove={handleCardDelete}/>
      <Footer/>
      <AddPlacePopup isClose={closeAllPopup} isOpen={isEditCardPopupOpen} handleSubmit={handleAddPlaceSubmit}>
      </AddPlacePopup>
      ...
      <EditAvatarPopup isOpen={isEditAvatarPopupOpen} isClose={closeAllPopup} onUpdateAvatar={handleUpdateAvatar}/> 
